...
export default function Vip5AdminPage() {
  ...
  const [endPage, setEndPage] = useState('10262');
  ...
  async function createRun() {
    try {
      ...
      const data = await post({ action: 'create_run', startPage: Math.max(Number(startPage) || 1, 1), endPage: Math.max(Number(endPage) || 10262, Number(startPage) || 1) });
  ...
  return (
    <div className="vip5-shell">
      ...
      <main>
        ...
        <section className="vip5-card runner-card"><div className="section-head"><div><span className="panel-kicker">Local runner</span><h2>คำสั่งสำหรับ VPS / aaPanel</h2></div><span className="source-badge">SERVICE ROLE ONLY</span></div><p>หน้าเว็บทำหน้าที่สร้างและดูสถานะ Run ส่วนการดึง 10,262 หน้าให้รันจากเครื่องที่มี Chromium</p><pre><code>cd runner{`\n`}npm install{`\n`}SUPABASE_URL="https://YOUR_PROJECT.supabase.co" \{`\n`}SUPABASE_SERVICE_ROLE_KEY="เก็บไว้ในเครื่อง runner เท่านั้น" \{`\n`}START_PAGE=1 END_PAGE=10262 npm start</code></pre><p className="muted">ถ้าหยุดกลางทาง ให้ใช้ <code>RUN_ID=&lt;run-id&gt;</code> เดิมเพื่อ resume จากหน้าล่าสุด</p></section>
