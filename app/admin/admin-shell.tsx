...
const nav = [["/admin", "ภาพรวม"], ["/admin/catalog", "จัดการรายการ"], ["/admin/test-all", "ทดสอบทุกเรื่อง"], ["/admin/health", "Player Health"], ["/admin/vip5", "AVDB VIP5"], ["/admin/player", "Player Lab"], ["/admin/tools", "เครื่องมือเดิม"], ["/admin/system", "ระบบ"]] as const;
...
export default function AdminShell({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  ...
  return <main className={styles.page}><div className={styles.shell}>
    <header className={styles.topbar}><div><p className={styles.kicker}>HLSHUB CONTROL ROOM</p><h1>{title}</h1><p className={styles.description}>{description}</p></div><div className={styles.topActions}><Link className={styles.topLink} href="/">ดูหน้าเว็บ ↗</Link><button className={styles.button} type="button" onClick={async () => { await fetch("/api/admin/logout", { method: "POST" }); window.location.href = "/"; }}>ออกจากระบบ</button></div></header>
