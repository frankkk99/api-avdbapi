...
export async function GET(request: NextRequest) {
  ...
  try { return NextResponse.json({ ok: true, overview: await fetchAdminCatalogOverview() }, { headers: { 'Cache-Control': 'no-store' } }); }
  catch (error) { return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : 'อ่านสถานะ catalog ไม่สำเร็จ' }, { status: 500 }); }
}
