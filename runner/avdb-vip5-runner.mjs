...
const SINGLE_PAGE = Math.max(Number(process.env.PAGE || 0), 0);
const DEFAULT_START = SINGLE_PAGE || Math.max(Number(process.env.START_PAGE || 1), 1);
const DEFAULT_END = SINGLE_PAGE || (process.env.END_PAGE ? Math.max(Number(process.env.END_PAGE), DEFAULT_START) : DEFAULT_START);
