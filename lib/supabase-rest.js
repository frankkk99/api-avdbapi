...
function getConfig(requireServiceRole = false) {
  ...
  const key = requireServiceRole ? serviceKey : publicKey;
  if (!key) throw new Error(requireServiceRole ? 'SUPABASE_SECRET_KEY or SUPABASE_SERVICE_ROLE_KEY is required' : 'Supabase public configuration is required');
